function [aTime, aTheta] = CPDataAnalysisM(filepath, filenames)
...
for i = 1:length(aTime)
    if aTime(i) >= 0.2667 && aTime(i) <= 0.4333
        sTime(end + 1) = aTime(i);
...
[b,a] = butter(2, 0.35, 'low'); %About 1/3 is the large spike on the freq
